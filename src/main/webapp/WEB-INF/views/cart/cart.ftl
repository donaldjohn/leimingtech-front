			...
			<tbody>
				...
				<#assign cartMap = cartTag() />
	       		<#if cartMap.get("cartVoList")??>
		            ...
		            <tr>
		              <th colspan="20">店铺：
		              <a href="${base}/store/shop?storeId=${cartVo.storeId}"  target="_Blank">${cartVo.storeName}</a></th>
		            </tr>
		            <#if cartVo.list??>
		            	<#list cartVo.list as cart>
				            <tr id="cart_item_${cart.goodsId}">
								...
								<td class="w70"><div class="cart-goods-pic"><a href="${base}/product/detail?id=${cart.goodsId}" target="_blank"><span class="thumb size60"><i></i><img src="${imgServer}${cart.goodsImages}" onload="javascript:DrawImage(this,60,60);" /></span></a></div></td>
				              	<td class="tl vt">
				         ...
				         <#assign promote = promoteTag("{'storeId':'${cartVo.storeId}'}")/>
				         <#if (promote!='')>
				         <tr>
				         	<td colspan="7"style="text-align:left;padding-left:10px;">
				         促销: <span style="color:red;">${promote}</span></td>
				         </tr>
				        </#if>
					</#list>
   				</#if>
