...
<form method="post" id="order_form" name="order_form" action="${base}/cart/subOrder">
	<input type="hidden" name="cartIds" value="${cartIds}" id="cartIds" /> 
  	<div class="content wrapper">
		...
		<div id="addresslist">
		<#assign promoteTag = newTag("promoteTag")/>
					...
					<#if address.isDefault == 1>
						<ul class="receive_add address_item selected_address">
			    			...
			    			<li address="${address.areaInfo}&nbsp;&nbsp;${address.address}" buyer="${address.trueName}&nbsp;&nbsp;${address.mobPhone}">
			      				<input id="address_${address.addressId}" isDefault="${address.isDefault}" type="radio" city_id="${address.cityId}" name="address_options" value="${address.addressId}" checked/>
			  			...
			  			<#else>
			  				<ul class="receive_add address_item">
				    			...
				    			<li address="${address.areaInfo}&nbsp;&nbsp;${address.address}" buyer="${address.trueName}">
				      				<input id="address_${address.addressId}" isDefault="${address.isDefault}" type="radio" city_id="${address.cityId}" name="address_options" value="${address.addressId}" />
				      				&nbsp;&nbsp;${address.areaInfo}&nbsp;&nbsp;${address.address}&nbsp;&nbsp; ${address.trueName}&nbsp;&nbsp; ${address.mobPhone}  
				      			</li>
				  			</ul>
				  			<input type="hidden" id="chooseaddressid" name="chooseaddressid" value='${address.addressId}'/>
    	...
    	<input type="hidden" name="storeId" value="${cartVo.storeId}" /> 
  			<table class="buy-table" nc_type="table_cart" >
    			...
    			<tbody>
      				<tr>
        				<th colspan="20">店铺：<a target="_blank" <#if (cartVo.storeId!=0)>href="${base}/store/shop?storeId=${cartVo.storeId}"<#else>href="javascript:void(0);"</#if> >${cartVo.storeName}</a>
        				<#assign promote = promoteTag("{'storeId':'${cartVo.storeId}','totalPrice':'${cartVo.goodsTotalPrice}'}")/>
				         <#if (promote!='')>
				         促销: <span style="text-align:right;padding-right:10px;color:red;">${promote}</span>
				        </#if>
        				</th>
      				</tr>
      				<#if cartVo.list??>
		            		...
		            		<tr id="cart_item_${cart.cartId}">
		        				<td class="w70">
		        					<div class="cart-goods-pic">
		        						<a href="${base}/product/detail?id=${cart.goodsId}" target="_blank" ><span class="thumb size60"><i></i><img src="${imgServer}${cart.goodsImages}" onload="javascript:DrawImage(this,60,60);" /></span></a>
		        					</div>
		        				</td>
		        				<td class="tl vt">
...
$(function(){	
    ...
    $(".address_item").hover(
    	function(){
    		if(!$(this).hasClass("selected_address")){
    			var isDefault = $(this).find("input[name='address_options']").attr("isDefault");
    			if(isDefault==0){
    				var id = $(this).find("input[name='address_options']").val();
    				$(this).append("<a style='color:blue;' onclick='updateDefAddress("+id+");'>设置为默认收货地址</a>");
    			}
    		}
    	},function(){
    		$(this).find("a").remove(); 
