...
<title>购物车列表页 - LmShop B2B2C V3.0 2015版商城系统</title>
...
function big_cart_remove(ids){
			var type=arguments[1];
      			var method = function(){
					   jQuery.post("http://b2b2c.iskyshop.com/remove_goods_cart.htm",{"ids":ids},function(data){
							if(type=="one_combin"){
...
function change_spec(obj){
	...
	var div =jQuery("#standard_"+obj_id);
		...
		jQuery.post("http://b2b2c.iskyshop.com/goods_cart1_spec.htm",{
			"cart_id":obj_id
			},function(data){
...
function confirm_change_spec(obj){
	...
	jQuery.post("http://b2b2c.iskyshop.com/goods_cart1_spec_save.htm",{
			"id":id,
			"gsp":gsp
			},function(data){
				if(data.code==100){
			        		...
			        		<div class="li_box">
				                ...
				                <div class="infro">
				   				...
				   				<div class="fr">
					   				...
					   				<span id="gc_totalprice${cart.cartId}">${cart.goodsNum*cart.goodsPrice}</span>
					   				<span class="del">
			 ...
			 if(id!="" && id!=undefined){
			   var dis = jQuery("#combin_other").css("display");
				   if(dis=="none"){
					   ...
					   jQuery.post("http://b2b2c.iskyshop.com/combin_carts_detail.htm",
								 {"id":id
									 },function(data){
										 if(data.code==100){
