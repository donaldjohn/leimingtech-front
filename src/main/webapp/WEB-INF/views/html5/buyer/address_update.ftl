...
<title>用户中心 - LmShop B2B2C V3.0 2015版商城系统</title>
...
function addAddress(){
	 ...
	 //验证电话号码
	 if(telPhone.trim()==''){
		 layer.open({content: '电话号码不能为空', time: 1});
         return false;
	 }else{
		 var tel = /^\d{3,4}-?\d{7,9}$/; //电话号码格式010-12345678 
	     if(!tel.test(telPhone)){
	    	 layer.open({content: '电话号填写不正确', time: 1});
	         return false;
	     }
	 }
	 ...
	 //验证邮编
	 if(zipCode.trim()==''){
		 layer.open({content: '请填写邮编号', time: 1});
	     return false;
	 }else{
		 var code = /^[0-9]{6}$/;
	     if(!code.test(zipCode)){
	    	 layer.open({content: '邮编填写不正确', time: 1});
	         return false;
	     }
	 }
	 ...
	 $.ajax({
	     ...
	     success:function(data){
	         if(data.success){
	        	  layer.open({content: '更新成功', time: 1});
	              if(data.page==1){
	            	  //购物车订单页
	        	 ...
	        	 layer.open({content: '更新失败', time: 1});
	         }
	     },error:function(data){
...
<body>
<div class="user_bar"><a class="back" href="javascript:history.go(-1);"><img src="${base}/res/html5/images/back_black.png" width="30" height="30" /></a><span class="fl">地址更新</span></div>
<div class="info_form">
    ...
    <li>
	    ...
	    <div id="region">
		    ...
		    <span id="spanarea">
		        ...
		        <#if areas??>
					<#list areas as str>
						<option <#if str.areaId==address.provinceId>selected="selected"</#if> value="${str.areaId}" >${str.areaName}</option>
					</#list>
				</#if>
			    </select> 	
		    </span>
