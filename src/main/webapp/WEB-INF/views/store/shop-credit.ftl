...
<#if storeVo??>
	  ...
	  <article id="content">
	    <section class="layout expanded mt10">
	      <article class="nc-goods-main"> 
	       <#assign evaluateTagMapTag = newTag("evaluateTag")/>
	         <#assign evaluateMap =evaluateTagMapTag("{'storeId':'${storeId}'}")>
	        <!-- 商品评分统计信息S -->
	        <div class="nc-s-c-s2">
	          <div class="title">
	            <h4>好评率 ( <#if evaluateMap ??&& evaluateMap!=null>
	                            <#if evaluateMap.get("sixmonthsumMakNum") ??&& evaluateMap["sixmonthgoodMarkNum"]!=0 && evaluateMap["sixmonthgoodMarkNum"]??>
	                                   ${(evaluateMap.get("sixmonthgoodMarkNum")*100/(evaluateMap.get("sixmonthsumMakNum")?number))?string("0.##")}
	                            <#else>
	                               0
	                            </#if> 
	                      <#else>
	            ...
	            <table class="ncs-evaluation-tb">
	              ...
	              <tbody>
	                <tr>
	                  <th>最近1周</th>
	                  <td class="ncse-good">
	                    <#if evaluateMap ??&& evaluateMap.get("oneweekgoodMarkNum")??>
	                            ${evaluateMap["oneweekgoodMarkNum"]}
	                    <#else>
	                        0
	                    </#if>
	                  </td>
	                  <td class="ncse-normal">
	                     <#if evaluateMap ??&& evaluateMap.get("oneweekmediumMarkNum")??>
	                            ${evaluateMap["oneweekmediumMarkNum"]}
	                    <#else>
	                        0
	                    </#if>
	                  </td>
	                  <td class="ncse-bad">
	                    <#if evaluateMap ??&& evaluateMap.get("oneweekbadMarkNum")??>
	                            ${evaluateMap["oneweekbadMarkNum"]}
	                    <#else>
	                        0
	                    </#if>
	                  </td>
	                  <td class="ncse-sum">
	                    <#if evaluateMap ??&& evaluateMap.get("oneweeksumMakNum")??>
	                            ${evaluateMap["oneweeksumMakNum"]}
	                    <#else>
	                        0
	                    </#if>
	                  </td>
	                ...
	                <tr>
	                  <th>最近1个月</th>
	                  <td class="ncse-good">
	                    <#if evaluateMap ??&& evaluateMap.get("onemonthgoodMarkNum")??>
	                            ${evaluateMap["onemonthgoodMarkNum"]}
	                    <#else>
	                        0
	                    </#if>
	                  </td>
	                  <td class="ncse-normal">
	                     <#if evaluateMap ??&& evaluateMap.get("onemonthmediumMarkNum")??>
	                            ${evaluateMap["onemonthmediumMarkNum"]}
	                    <#else>
	                        0
	                    </#if>
	                  </td>
	                  <td class="ncse-bad">
	                    <#if evaluateMap ??&& evaluateMap.get("onemonthbadMarkNum")??>
	                            ${evaluateMap["onemonthbadMarkNum"]}
	                    <#else>
	                        0
	                    </#if>
	                  </td>
	                  <td class="ncse-sum">
	                    <#if evaluateMap ??&& evaluateMap.get("onemonthsumMakNum")??>
	                            ${evaluateMap["onemonthsumMakNum"]}
	                    <#else>
	                        0
	                    </#if>
	                  </td>
	                ...
	                <tr>
	                  <th>最近6个月</th>
	                  <td class="ncse-good">
	                    <#if evaluateMap ??&& evaluateMap.get("sixmonthgoodMarkNum")??>
	                            ${evaluateMap["sixmonthgoodMarkNum"]}
	                    <#else>
	                        0
	                    </#if>
	                  </td>
	                  <td class="ncse-normal">
	                     <#if evaluateMap ??&& evaluateMap.get("sixmonthmediumMarkNum")??>
	                            ${evaluateMap["sixmonthmediumMarkNum"]}
	                    <#else>
	                        0
	                    </#if>
	                  </td>
	                  <td class="ncse-bad">
	                    <#if evaluateMap ??&& evaluateMap.get("sixmonthbadMarkNum")??>
	                            ${evaluateMap["sixmonthbadMarkNum"]}
	                    <#else>
	                        0
	                    </#if>
	                  </td>
	                  <td class="ncse-sum">
	                    <#if evaluateMap ??&& evaluateMap.get("sixmonthsumMakNum")??>
	                            ${evaluateMap["sixmonthsumMakNum"]}
	                    <#else>
	                        0
	                    </#if>
	                  </td>
	          ...
	          <div class="content">
	            <div class="ncs-rate clearfix">
	              <ul class="ncs-rate-tab">
	                ...
	                <li><a href="javascript:void(0);">卖家的服务态度：&nbsp;&nbsp;<em>${evaluateStore.sevalServicecredit!'0'}</em>分</a></li>
	                <li><a href="javascript:void(0);">卖家的发货速度：&nbsp;&nbsp;<em>${evaluateStore.sevalDeliverycredit!'0'}</em>分</a></li>
	              </ul>
	...
	<div class="ncs-info clearfix">
	  <div class="shop-card">
	    ...
	    <dl>
	      ...
	      <dd class="ncs-level">
	        卖家信用：${evaluateStore.averageCredit!'0'}</dd>
	  ...
	  <div class="shop-other" id="shop-other">
	    <ul>
	      ...
	      <li class="ncs-info-btn-qrcode"><a href="javascript:void(0)" class="pngFix"><span>店铺二维码</span><b></b> <!-- 店铺二维码 -->
	        <p class="ncs-info-qrcode"><img src="${imgServer}${storeVo.storeCode}"  title="#" onload="javascript:DrawImage(this,150,150);"><em>手机扫描二维码<br/>快速收藏店铺</em> </p>
	        </a> </li>
	    </ul>
	  </div>
	</div>
	...
	<div class="wrapper">
	  <p><a href="${base}">首页</a>
	                ...
	                | <a  href="#?act=article&amp;article_id=22">关于我们</a>
	                            </p>Copyright 2014-2018 北京雷铭信科技有限公司 Inc.All rights reserved.&nbsp;&nbsp;ICP证：<br/>
   ...
   <#if storeVo.storeState==0>
      <div align="center">
        <b style="font-size: 3em;">该店铺已关闭</b>
      </div>
   </#if>
   <#if storeVo.storeState==2>
       <div align="center">
     	 <b style="font-size: 3em;">该店铺正在审核中</b>
       </div>
   </#if>
</#if>
