...
<div class="public-top-layout w">
    <div class="topbar wrapper">
        ...
        <@shiro.user>
            您好<span>&nbsp;<a href="${base}/user/index/"><@shiro.principal/></a></span>，欢迎来到<a href="${base}"  title="首页" alt="首页">雷铭商城系统演示站</a>
            <span>[<a href="${base}/logout">退出</a>]</span>
        </@shiro.user>
        <@shiro.guest>
            您好，欢迎来到<a href="${base}" title="首页" alt="首页">雷铭商城系统演示站</a>
            <span>[<a href="login">登录</a>]</span>
            <span>[<a href="signUp">注册</a>]</span>
        </@shiro.guest>
	  <span class="seller-login">
	<a href="${sellerServer}" target="_top" title="商家中心" target="_blank" title="登录商家管理中心"><i
        ...
        <div class="quick-menu">
            <dl>
                ...
                <dd>
                    <ul>
                        <li><a href="#?act=member&op=order&state_type=order_pay">待付款订单</a>
                        </li>
                        <li>
                            <a href="#?act=member&op=order&state_type=order_shipping">待确认收货</a>
                        </li>
                        <li><a href="#?act=member&op=order&state_type=noeval">待评价交易</a>
                        </li>
                    </ul>
                </dd>
            </dl>
            <dl>
                <dt><a href="#?act=member_favorites&op=fglist" title="我的收藏" target="_top">我的收藏</a><i></i>
                </dt>
                <dd>
                    <ul>
                        <li><a href="#?act=member_favorites&op=fglist" target="_top"
                               title="收藏的商品">收藏的商品</a></li>
                        <li><a href="#?act=member_favorites&op=fslist" target="_top"
                               title="收藏的店铺">收藏的店铺</a></li>
                    </ul>
                </dd>
            </dl>
            <dl>
...
<header id="header" class="pngFix">
    <div class="wrapper">
        <h1 id="logo" title="雷铭商城系统演示站"><a href="${base}"><img
                src="${base}/res/images/common/690ea902fe2708381da2cabff4ee46c8.png" alt="雷铭商城系统演示站" class="pngFix"></a></h1>
...
<div id="container" class="wrapper mt20">
    ...
    <div class="sns-nav">
        <ul>
            ...
            <li><a href="#?act=member_snshome&op=shareglist&mid=7"><i class="goods pngFix"></i>宝贝</a></li>
            <li><a href="#?act=member_snshome&op=storelist&mid=7"><i class="store pngFix"></i>店铺</a></li>
            ...
            <script>
                ...
                function leaveMsgSuccess(data) {
                    $('<dl></dl>')
                            ...
                            .append('<dd data-param="{\'msgid\':\'' + data.msg_id + '\',\'fname\':\'' + data.from_member_name + '\'}"><span class="time">刚刚</span><span class="handle"><a href="javascript:void(0);"onclick="ajax_get_confirm(\'您确定要删除该信息吗？\', \'#?act=home&op=dropcommonmsg&drop_type=sns_msg&message_id=' + data.msg_id + '\');">删除</a></span></dd>')
            ...
            $(function () {
                ...
                $("[nc_type='storedelbtn']").live('click', function () {
                    ...
                    showDialog('您确定要删除该信息吗？', 'confirm', '', function () {
                        ajax_get_confirm('', '#?act=member_snsindex&op=delstore&id=' + data_str.sid);
                        return false;
                    });
                });
            });
...
<script language="javascript">
    ...
    function load_cart_information() {
        $.getJSON('#?act=cart&op=ajaxcart', function (result) {
            if (result) {
                var result = result;
                ...
                if (result.goods_all_num > 0) {
                    ...
                    for (i = 0; i < data.length; i++) {
                        ...
                        html += "<dt class='goods-name'><a href='#?act=goods&goods_id=" + data[i]['goodsid'] + "' title='" + data[i]['gname'] + "' target='_top'>" + data[i]['gname'] + "</a></dt>";
                    ...
                    html += "<div colspan='3' class='checkout'><span class='total-price'>共<i>" + result.goods_all_num + "</i>种商品   金额总计：<em>&yen;" + result.goods_all_price + "</em></span><span class='btn-cart' ><a href='#?act=cart' target='_top' title='结算商品' style='color: #FFF;' >结算购物车中的商品</a></span></div>";
                } else {
...
<script type="text/javascript" language="javascript">
    ...
    $(function () {
        ...
        $('a[nctype="change_skin"]').click(function () {
            ajax_form('change_skin', '选择皮肤', SITEURL + '/#?act=sns_setting&op=change_skin', '480');
        });
    });
</script>
</body>
