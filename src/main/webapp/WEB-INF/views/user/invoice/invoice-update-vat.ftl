		...
		<script>
			$(function(){
				<#list invoices as invoice>
					...
					$("[name=invContent]").attr("value",'2');
				</#list>
				var areas = $("[name=invRecProvince]").val();
				if(areas!=""){
					var address = areas.toString().split(",");
					init_area(address[0],address[1],address[2]);
					$("#area").val(address[0]);
					$("#city_id").val(address[1]);
				 	$("#area_id").val(address[2]);
				}
			});
		</script>
	</#if>
					...
					<div class="item">
						...
						<div class="fl">
							<div class="invoice-list">
								<input type="hidden" name="invContent" value="2"/>
								<ul id="electro_book_content_radio">
									<li class="invoice-item" id="electro-invoice-content-1" name="normal-normalContent" value="1" style="cursor:pointer" onclick="select(this);">
										不开发票<b></b>
									</li>
									<li class="invoice-item invoice-item-selected" id="electro-invoice-content-2" name="normal-normalContent" value="2" style="cursor:pointer" onclick="select(this);">
										明细<b></b>
									</li>
								</ul>
							</div>
