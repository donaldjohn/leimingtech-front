...
public class ArticleTag extends BaseFreeMarkerTag {
	...
	protected Object exec(Map params) throws TemplateModelException {
		int articleId = ParamsUtils.getInt(params.get("articleId"));//文章id
		...
		//文章显示状态
		String articleShow = ParamsUtils.getString(params.get("articleShow"));
		...
		if(TagsDataType.PAGE_LIST.equals(tagType)){
			...
			if(StringUtils.isNotBlank(articleShow)){
				article.setArticleShow(Integer.valueOf(articleShow));
			}
			...
			if(articleId != 0){
				Article a = articleService.findById(Integer.valueOf(articleId));
				return a;
			}else{
				return null;
			}
