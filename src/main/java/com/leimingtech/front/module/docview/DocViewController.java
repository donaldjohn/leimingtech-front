package com.leimingtech.front.module.docview;
...
public class DocViewController {
	...
	private static final Map<Integer, String> returnType = new HashMap<Integer, String>();
	static{
		returnType.put(1, "Integer");
		returnType.put(2, "String");
		returnType.put(3, "Double");
		returnType.put(4, "Datetime");
		returnType.put(5, "Boolean");
		returnType.put(6, "List");
		returnType.put(7, "bean");
	}
	
	private static final Map<Integer, String> requiredMap = new HashMap<Integer, String>();
	static{
		requiredMap.put(1, "是");
		requiredMap.put(0, "否");
	}
	...
	public String docapi(
		...
		try {
			...
			// 获取当前api下的返回值
			List<DocReturnValueEntity> returnList = docReturnValueServiceImpl.getReturnValueListByDocId(id);
			for (DocReturnValueEntity docReturnValueEntity : returnList) {
				docReturnValueEntity.setTypename(returnType.get(Integer.parseInt(docReturnValueEntity.getType())));
				docReturnValueEntity.setRequiredvalue(requiredMap.get(docReturnValueEntity.getIsrequired()));
				if(StringUtils.isNotEmpty(docReturnValueEntity.getCode())){
					DocEnEntity entity = docEntityService.getDocEntityByName(docReturnValueEntity.getCode());
					docReturnValueEntity.setDocEntityId(entity.getId());
				}
			}
			
			model.addAttribute("docApi", docApi);
			model.addAttribute("paramList", paramList);
			model.addAttribute("returnValue", returnList);
	 ...
	 * @return
	 */
	@RequestMapping("/doctagdetail")
	public String tagapi(
			Model model,
			@RequestParam(value = "id", required=false, defaultValue = "0") int id,
			@RequestParam(value = "typeid", required=false, defaultValue = "2") int typeid
			){
		try {
			// 获取当前api
			DocEntity docApi = new DocEntity();
