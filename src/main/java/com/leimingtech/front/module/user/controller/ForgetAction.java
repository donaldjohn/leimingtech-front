...
//	private MessageService messageService;
	...
	public @ResponseBody Map<String, String> sendValid(@RequestParam(value = "mobile") String mobile,
			@RequestParam(value = "currStr") String currStr,
			Model model)  {
		Map<String, String> map = Maps.newHashMap();
		Integer valid = NumberUtils.getRandomInt(99999);
		//预留短信接口 msnServiceImpl.send(valid);
	    try {
				Integer membercount=memberService.findMemberCountByMobile(mobile);
				if(membercount==0){
					map.put("success", "false");
					...
					C123SendSmsUtil sms=new C123SendSmsUtil();//发送短信
					String str=sms.sendSms(mobile,"短信验证码："+valid+"。");
					if("1".equals(str)){
						map.put("success", "true");
	...
	public ModelAndView toRest(@RequestParam(value = "mobilep") String mobilep,
		...
		try {
			...
			if(member==null){
				...
				model.addObject("error", "用户不存在找回密码失败！");
				return model;
			}
			model.addObject("member", member);
